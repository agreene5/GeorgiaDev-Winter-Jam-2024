...
func _ready():
	speed = randf_range(min_speed, max_speed) # Gives the elf a random speed between the set r
...
func _physics_process(delta):
	...
	for elf in get_tree().get_nodes_in_group("Elves"): # Checks for nearby elves and if its too close it moves backwards for 5 seconds
		if elf != self:  # Making sure the elf isn't detecting itself
...
func flip_and_reverse():
	...
	is_flipped = true
	flip_timer = 5.0  # Stay flipped and move in opposite direction for 5 seconds
...
func reset_movement():
	if is_flipped:
		...
		is_flipped = false
		cooldown_timer = 2.0  # Have a 2 second cooldown before detecting another elf
